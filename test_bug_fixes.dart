...
void main() async {
  ...
  try {
    ...
    if (result.totalScannedFiles > 0) {
      final percentUnused =
          (result.totalUnusedItems / result.totalScannedFiles) * 100;
      print(
          '  • Percentage marked unused: ${percentUnused.toStringAsFixed(1)}%');

      if (percentUnused < 80) {
        print('✅ PASS: Reasonable number of items marked as unused');
      } else {
        print(
            '❌ FAIL: Too many items marked as unused (${percentUnused.toStringAsFixed(1)}%)');
      }
    }
    ...
    for (final file in protectedFiles) {
      final isMarkedUnused =
          result.unusedFiles.any((item) => item.path.endsWith(file));
      if (isMarkedUnused) {
...
''');
  ...
  await File('${testDir.path}/assets/images/icon.png')
      .writeAsBytes([1, 2, 3, 4]);

  // Declared config (should be protected)
  await File('${testDir.path}/assets/config.json')
      .writeAsString('{"version": "1.0"}');

  // Undeclared asset (should be marked as unused)
  await File('${testDir.path}/assets/images/unused.png')
  ...
  await File('${testDir.path}/assets/extra/unused_extra.json')
